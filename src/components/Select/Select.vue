<template>
  ...
  >
    ...
    >
      ...
      <template #content>
        <div v-if="states.loading" class="lyz-select__loading">
          <Icon icon="spinner" spin />
        </div>
        <div v-else-if="filterable && filteredOptions.length === 0" class="lyz-select__noData">
          no matching data
        </div>
        <ul v-else class="lyz-select__menu">
          <template v-for="(item, index) in filteredOptions" :key="index">
            <li
              :id="`select-item-${item.value}`"
              class="lyz-select__menu-item"
              :class="{
